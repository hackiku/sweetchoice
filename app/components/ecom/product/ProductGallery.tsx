...
export function ProductGallery({ images, title }: ProductGalleryProps) {
	...
	if (!images?.length) {
		return (
			<div className="product-image border-4 border-black rounded-xl shadow-[8px_8px_0px_rgba(0,0,0,1)] 
                    bg-gradient-to-br from-[#FFF9E5] to-[#FFE4E1] aspect-square flex items-center justify-center">
	...
	return (
		<div className="flex flex-col gap-4">
			{/* Main Product Image - Enhanced */}
			<div className="product-image border-4 border-black rounded-xl shadow-[8px_8px_0px_rgba(0,0,0,1)] 
                    bg-gradient-to-br from-[#FFF9E5] via-white to-[#FFE4E1] aspect-square overflow-hidden
				...
				<div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
					{images.map((image, index) => (
						<button
							...
							className={`flex-none w-20 h-20 border-3 border-black rounded-lg overflow-hidden
                         transition-all duration-200 hover:scale-105
                         ${selectedImage === index
									? 'ring-4 ring-[#FF6B6B] shadow-[4px_4px_0px_rgba(255,107,107,1)] scale-105'
